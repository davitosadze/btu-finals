...
<div class="row">
  ...
  @if(count($question->options) < 4)
    <a class="quiz-add-button btn btn-success" href="{{route('user.quizzes.questions.options.add', $question->id)}}">პასუხის დამატება</a>
  @else
        ...
        <tbody>
          @foreach ($question->options as $option)
            <tr>
                ...
                <td><a class="btn btn-success" href="{{route('user.quizzes.questions.options.edit', $option->id)}}">რედაქტირება</a></td>
                <td>
                
                  <form action="{{ route('user.quizzes.questions.options.delete', $option->id)}}" method="post">
                    @method('delete')
                    @csrf
